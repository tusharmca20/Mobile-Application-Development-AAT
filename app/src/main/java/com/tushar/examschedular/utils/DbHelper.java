package com.tushar.examschedular.utils;
...
import com.tushar.examschedular.fragments.WeekdayFragment;
import com.tushar.examschedular.model.Exam;
import com.tushar.examschedular.model.Homework;
import com.tushar.examschedular.model.Note;
import com.tushar.examschedular.model.Teacher;
import com.tushar.examschedular.model.Week;
...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String DB_NAME = "examschedulardb";

    private static final String examschedular = "examschedular";
    private static final String examschedular_ODD = "examschedular_odd";
    ...
    public void onCreate(@NonNull SQLiteDatabase db) {
        String CREATE_examschedular = "CREATE TABLE IF NOT EXISTS " + examschedular + "("
        ...
        String CREATE_examschedular_ODD = "CREATE TABLE IF NOT EXISTS " + examschedular_ODD + "("
                + WEEK_ID + " INTEGER PRIMARY KEY AUTOINCREMENT,"
                + WEEK_SUBJECT + " TEXT,"
                + WEEK_FRAGMENT + " TEXT,"
                + WEEK_TEACHER + " TEXT,"
        ...
        db.execSQL(CREATE_examschedular);
        db.execSQL(CREATE_examschedular_ODD);
    ...
    private void migrateEvenOddWeeks(SQLiteDatabase db) {
        ...
        for (String key : keys) {
            oldOddWeeks.addAll(oldDbHelper.getWeek(key, examschedular));
        }

        for (Week week : oldOddWeeks) {
            insertWeek(week, examschedular_ODD, db);
        }
    }
     ...
     **/
    private String getexamschedularTable() {
        return getexamschedularTable(Calendar.getInstance());
    }

    private String getexamschedularTable(Calendar now) {
        if (PreferenceUtil.isEvenWeek(context, now))
            return examschedular;
        else
            return examschedular_ODD;
    }

    public void insertWeek(Week week) {
        insertWeek(week, getexamschedularTable(), this.getWritableDatabase());
    }
...
//        db.close();
    ...
    public void deleteWeekById(@NonNull Week week) {
        ...
        db.delete(getexamschedularTable(), WEEK_ID + " = ? ", new String[]{String.valueOf(week.getId())});
        db.close();
    }
    ...
    public void updateWeek(@NonNull Week week) {
        ...
        contentValues.put(WEEK_TO_TIME, week.getToTime());
        contentValues.put(WEEK_COLOR, week.getColor());
        db.update(getexamschedularTable(), contentValues, WEEK_ID + " = " + week.getId(), null);
        db.close();
    }
    ...
    public ArrayList<Week> getWeek(String fragment, Calendar now) {
        return getWeek(fragment, getexamschedularTable(now));
    }
    ...
    public void deleteAll() {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DROP TABLE IF EXISTS " + examschedular);
        db.execSQL("DROP TABLE IF EXISTS " + examschedular_ODD);
